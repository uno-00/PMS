<div>
    ...
    <x-card>
        <x-table.filter-toolbar>Use the column filters below to search BAC member records.</x-table.filter-toolbar>
            ...
            <table class="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-800">
                <thead>
                    ...
                    <tr class="border-b border-slate-100 dark:border-slate-800">
                        <th class="pb-3 pr-4 pt-1"><x-table.filter-text model="filterSearch" placeholder="Name or email…" /></th>
                        <th class="pb-3 pr-4 pt-1">
                            <x-table.filter-select model="filterRole" placeholder="All roles">
                        ...
                        <th class="pb-3 pr-4 pt-1">
                            <x-table.filter-select model="filterCategory" placeholder="All categories">
                                @foreach($twgCategories as $category)
                                    <option value="{{ $category->value }}">{{ $category->label() }}</option>
                                @endforeach
                            </x-table.filter-select>
                        </th>
                        <th class="pb-3 pr-4 pt-1"><x-table.filter-text model="filterDesignation" /></th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                    @forelse($members as $member)
                        <tr wire:key="bac-member-{{ $member->id }}">
                            ...
                            <td class="py-3 pr-4 text-xs text-slate-500">{{ $member->created_at?->format('M d, Y') }}</td>
