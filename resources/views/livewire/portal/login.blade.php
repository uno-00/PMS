<div x-data="{ showPassword: false }">
    <div class="mb-8">
        <h2 class="text-2xl font-bold text-slate-900 dark:text-white">Supplier / Bidder Sign In</h2>
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Access your bidding dashboard, orders, and awards.</p>
    </div>
    ...
    <form wire:submit="login" class="space-y-5">
        <div>
            ...
            <div class="mt-1.5 flex items-stretch overflow-hidden rounded-xl border border-slate-300 bg-white shadow-sm transition focus-within:border-indigo-500 focus-within:ring-2 focus-within:ring-indigo-500/40 dark:border-slate-700 dark:bg-slate-900">
        ...
        <button type="submit"
                class="flex w-full items-center justify-center rounded-lg bg-indigo-700 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-indigo-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
    ...
    <p class="mt-8 text-center text-xs text-slate-400">
        New supplier? <a href="{{ route('bidder.register') }}" class="font-medium text-indigo-600 hover:underline">Register your company</a>
        &middot; Agency staff? <a href="{{ route('login') }}" class="font-medium text-indigo-600 hover:underline">Staff login</a>
    </p>

    @if ($this->demoAccountsEnabled)
        <div class="mt-8 rounded-lg border border-dashed border-slate-300 px-4 py-3 dark:border-slate-700">
            <p class="text-xs text-slate-500 dark:text-slate-400">
                Demo / test account (UAT only) &mdash; password:
