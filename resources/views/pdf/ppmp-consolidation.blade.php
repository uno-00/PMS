...
<head>
    <meta charset="utf-8">
    <title>Consolidated PPMP No. {{ $ppmpNumber }} — {{ $agency->displayName() }}</title>
    <style>
...
<body>
    ...
    <div class="form-title">Consolidated Project Procurement Management Plan (PPMP) No. {{ $ppmpNumber }}</div>
    ...
    <table class="meta-row" style="margin-bottom: 6px;">
        <tr>
            <td style="width: 50%;"><span class="meta-label">Fiscal Year:</span> {{ $consolidation->fiscalYear?->year ?? '—' }}</td>
            <td style="width: 50%;"><span class="meta-label">End-User or Implementing Unit:</span> Agency-wide (Consolidated)</td>
        </tr>
        <tr>
            <td colspan="2"><span class="meta-label">PPMP Title:</span> {{ $consolidation->title }}</td>
        </tr>
        <tr>
            <td><span class="meta-label">Control No.:</span> {{ $consolidation->reference_no }}</td>
            <td><span class="meta-label">Status:</span> {{ $consolidation->status->label() }}</td>
        </tr>
        <tr>
            <td colspan="2"><span class="meta-label">Source PPMPs:</span> {{ $consolidation->sourcePpmps->pluck('control_no')->filter()->join(', ') ?: '—' }}</td>
        </tr>
    </table>
    ...
    <table class="ppmp-table">
        ...
        <tfoot>
            <tr>
                ...
                <td class="text-right" style="font-weight: bold;">{{ $formatter::totalBudget($consolidation) }}</td>
                <td colspan="2"></td>
            </tr>
        </tfoot>
    </table>
    ...
    <table class="signatures">
        <tr>
            <td>
                ...
                <div class="sig-line">{{ $consolidation->creator?->name ?? '_______________________________' }}</div>
                <div class="sig-caption">Signature over Printed Name / Position / Designation<br>[Planning Office / End-User or Implementing Unit]</div>
    ...
    <div class="footer-note">
        ...
        Consolidated PPMP aligned with GPPB NGPA PPMP (RA 12009 IRR Sec. 7.7.1–7.7.3).
    </div>
</body>
</html>
